...
class RoomSerializer(serializers.ModelSerializer):
    ...
    def validate_day_price(self, value):
        if value <= 0:
            raise serializers.ValidationError(
                'day price should be higher than zero')
    ...
    def validate_bed_amount(self, value):
        if value < 1:
            raise serializers.ValidationError(
                'bed amount should be at least one')
...
class ReservationDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = ['id', 'id_customer', 'id_room',
                  'start_date', 'end_date', 'daycost', 'totalcost']
...
class ReservationHyperlinkedSerializer(serializers.HyperlinkedModelSerializer):
    ...
    id_room = serializers.HyperlinkedRelatedField(
...
class ReportDetailedSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Report
        fields = ['id', 'owner', 'reservations', 'reservation_count', 'total_price',
                  'total_rooms',  'avg_price', 'avg_rooms', 'unique_customers']
